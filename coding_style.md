...
- **简洁，敏捷开发**：不需要写太稳健的代码，比如各种错误处理、特殊情况判断等，先写出MVP，需要时再往里加；时刻牢记第一性原理。
...
## 5. 给 AI 的“业务上下文底座”
- 每次关闭 codex/与 AI 对话时，
维护：`docs/diary/{对话日期}_context.md`
- 对于任务计划的执行情况，需要及时更新在TASK.md中，便于跟踪，希望以多个表格的形式呈现。第一个表格为各个主任务完成情况，后续每个表格为每个主任务的子任务完成情况。表头为拆分出的子任务、状态等
