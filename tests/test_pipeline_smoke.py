...
def test_run_demo_pipeline_writes_outputs() -> None:
    ...
    assert (out_dir / "blacklist_features.parquet").exists()
    ...
    blacklist_features = pl.read_parquet(out_dir / "blacklist_features.parquet")
    assert {"ever_default_flag", "default_cnt_36m"}.issubset(set(blacklist_features.columns))
